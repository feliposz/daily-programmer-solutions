...
hexen = ['', 'ten', 'twenty', 'thirty', 'fourty',
         'fifty', 'sixty', 'seventy', 'eighty', 'ninety',
         'atta', 'bibbity', 'city', 'dickety', 'ebbity', 'fleventy']

def pronounce_hex(num):
    assert 1 <= num <= 65535, 'Implemented only from 0x0001 to 0xFFFF'
    if num >= 256:
        bitey_part = pronounce_hex(num // 256)
        rest = pronounce_hex(num % 256)
        if bitey_part and rest:
            return '%s bitey %s' % (bitey_part, rest)
    else:
        hexen_part = hexen[num//16]
        units_part = units[num%16]
        if hexen_part and units_part:
            return '%s-%s' % (hexen_part, units_part)
        elif hexen_part:
            return hexen_part
        elif units_part:
            return units_part
...
#    print('%i: %s' % (i, word))
#
#for i, word in enumerate(hexen):
#    print('%i: %s' % (i, word))

def test():
    assert pronounce_hex(0xF5) == 'fleventy-five'
    assert pronounce_hex(0xB3) == 'bibbity-three'
    assert pronounce_hex(0xE4) == 'ebbity-four'
    assert pronounce_hex(0xBBBB) == 'bibbity-bee bitey bibbity-bee'
    assert pronounce_hex(0xA0C9) == 'atta bitey city-nine'
